...
struct NotificationsSetupView: View {
    ...
    var body: some View {
        VStack(spacing: AppSpacing.lg) {
            ...
            PillButton(title: "Allow & Save") {
                scheduleNotifications(count: count)
                onContinue()
            }
            .padding(.horizontal, AppSpacing.lg)
    ...
    private func scheduleNotifications(count: Int) {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .badge, .sound]) { granted, _ in
            ...
            // Spread notifications between 9am and 10pm
            let startHour = 9
            let span = 13  // hours between 9 and 22
            ...
            for i in 0..<count {
                ...
                let request = UNNotificationRequest(identifier: "verbum_\(i)", content: content, trigger: trigger)
